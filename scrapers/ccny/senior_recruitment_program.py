...
TARGET_URL = "https://www.ccny.cuny.edu/cpdi/professional-recruitment-programs"
OUTPUT_DIR = "data_demo/ccny/student_tips"
OUTPUT_FILENAME = os.path.join(OUTPUT_DIR, "senior_recruitment_program.csv")

HEADERS = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
}

def scrape_senior_recruitment_program():
    ...
    try:
        ...
        if not content_area:
            print("Error: Could not find the main content area with class 'CS_Textblock_Text'.")
            return
        ...
        if first_p:
            scraped_data.append({
                'tip_title': 'Program Overview',
                'tip_text': ' '.join(first_p.get_text(strip=True, separator=' ').split())
            })
        ...
        for heading in all_headings:
            ...
            for sibling in heading.find_next_siblings():
                ...
                if sibling.name == 'p' and "APPLY NOW" in sibling.get_text():
                    continue
...
if __name__ == "__main__":
    scrape_senior_recruitment_program()