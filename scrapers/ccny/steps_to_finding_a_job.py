...
TARGET_URL = "https://www.ccny.cuny.edu/cpdi/steps-to-finding-a-job"
OUTPUT_DIR = "data_demo/ccny/student_tips"
OUTPUT_FILENAME = os.path.join(OUTPUT_DIR, "steps_to_finding_a_job.csv")

HEADERS = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
}

def scrape_steps_to_finding_a_job():
    ...
    try:
        ...
        all_headings = [p for p in content_area.find_all('p') if p.find('strong')]

        intro_content = []
        for element in content_area.find_all(['p', 'ul'], recursive = False):
            if element in all_headings:
                break
            
            if element.name == 'ul':
                for item in element.find_all('li'):
                    intro_content.append(f"- {item.get_text(strip = True)}")
            else:
                intro_content.append(element.get_text(strip = True))
        
        if intro_content:
            scraped_data.append({
                'tip_title': 'Job Search Overview',
                'tip_text': '\n'.join(intro_content)
            })
